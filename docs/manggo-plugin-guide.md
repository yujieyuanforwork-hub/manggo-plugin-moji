...
本仓库根目录本身就是一个完整的 Manggo 原生插件：基于 MOJi辞書 公开接口的日语词典、发音与划词插件。可参考 [`manggo.plugin.json`](../manggo.plugin.json) 中翻译（`resultType: "dictionary"`）、语音和划词动作的多服务声明，以及 [`main.js`](../main.js) 的网络请求、结构化词典返回值和 mp3 音频返回实现；运行 [`package.sh`](../package.sh) 可在 `dist/` 下生成 `.mplugin`。
